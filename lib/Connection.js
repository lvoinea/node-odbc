...
class Connection {
   ...
   * @param {Array<Array<number|string>>} values 
   * @param {Number} bufferSize 
   * @param {(error: NodeOdbcError, result: Result<unknown>)=> undefined} callback 
   */
  bulkInsert(table, columns, values, bufferSize, callback) {
    // promise...
    if (callback === undefined) {
      ...
      return new Promise((resolve, reject) => {
        this.odbcConnection.bulkInsert(table, columns, values, bufferSize, (error, result) => {
          if (error) {
            reject(error);
          } else {
            resolve(result);
    ...
    if (!this.odbcConnection) {
      ...
      this.odbcConnection.bulkInsert(table, columns, values, bufferSize, callback);
    }
  }
